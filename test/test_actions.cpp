...
TEST(EmailActuatorTest, SendsEmail) {
  ...
  EXPECT_NE(output.find(
                "[EMAIL] Sending alert to test@example.com: Test email alert"),
            std::string::npos);
...
TEST(ChargeRateActuatorTest, RestrictsChargeRate) {
  ...
  EXPECT_NE(output.find("[CHARGE RATE] Restricting charging rate to 60% due to "
                        "alert: Test charge restriction alert"),
            std::string::npos);
